...
export const StaffService = {
  ...
  create(data: StaffCreateDTO): Staff {
    const db = getDatabase();
    const stmt = db.prepare("INSERT INTO staff (name, active, is_parttime) VALUES (?, ?, ?)");
    const result = stmt.run(data.name, data.active ?? 1, data.is_parttime ?? 0);
  ...
  update(id: number, data: StaffUpdateDTO): Staff | undefined {
    ...
    db.prepare("UPDATE staff SET name = ?, active = ?, is_parttime = ? WHERE staff_id = ?").run(
